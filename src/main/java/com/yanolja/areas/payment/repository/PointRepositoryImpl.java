...
public class PointRepositoryImpl implements PointRepositoryCustom {
    ...
    @Override
    public List<Point> findExpiringPoints(PointStatus status, 
                                         PointTransactionType transactionType,
                                         LocalDateTime startTime, 
                                         LocalDateTime endTime) {
        return queryFactory
                .selectFrom(point)
                .where(point.status.eq(status)
                        .and(point.transactionType.eq(transactionType))
                        .and(point.expiredAt.between(startTime, endTime)))
                .fetch();
    }

    @Override
    public List<Point> findPointHistoryByDateRange(Long userId,
                                                  PointTransactionType transactionType,
        ...
        return queryFactory
                ...
                .where(point.userId.eq(userId)
                        .and(point.transactionType.eq(transactionType))
                        .and(point.createdAt.between(startDate, endDate)))
